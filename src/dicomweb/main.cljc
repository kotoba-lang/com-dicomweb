(ns dicomweb.main
  "Kotodama WASM entrypoint for the Dicomweb clean-room actor (L4) — Clojure port.
  ...
  facts (`dicomweb.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "dicomweb")
(def tier "L4")
...
(def entity-specs
  [{:entity "Study"      :plural "studies"      :id-prefix "dicomweb_stu"
    :fields [:studyInstanceUid :studyDate :studyTime :studyDescription :accessionNumber :referringPhysicianName]
    :required [:studyInstanceUid :studyDate]
    :coerce {}                       :refs {}}
   {:entity "Series"     :plural "serieses"     :id-prefix "dicomweb_ser"
    :fields [:seriesInstanceUid :seriesNumber :seriesDescription :modality :seriesDate]
    :required [:seriesInstanceUid :seriesNumber]
    :coerce {:seriesNumber :int}     :refs {}}
   {:entity "Instance"   :plural "instances"    :id-prefix "dicomweb_ins"
    :fields [:sopInstanceUid :sopClassUid :instanceNumber :contentDate]
    :required [:sopInstanceUid :sopClassUid]
    :coerce {:instanceNumber :int}   :refs {}}
   {:entity "Patient"    :plural "patients"     :id-prefix "dicomweb_pat"
    :fields [:patientId :patientName :patientBirthDate :patientSex]
    :required [:patientName :patientBirthDate]
    :coerce {}                       :refs {:patientId "Patient"}}
   {:entity "QidoQuery"  :plural "qidoqueries"  :id-prefix "dicomweb_qid"
    :fields [:patientId :patientName :studyInstanceUid :modality :fuzzyMatching]
    :required [:patientName :studyInstanceUid]
    :coerce {:fuzzyMatching :bool}   :refs {:patientId "Patient"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"dicomweb.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dicomweb-compat" :tier tier :entities entities} 200])
...
(defn start! [] :dicomweb-compat/ready)
